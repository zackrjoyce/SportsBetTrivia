...
function didThisPlayScoreFor(player, play) {
  ...
  // HARD GATE: if play text doesn't literally say TD, no one scored
  const detail = String(play.detail || "");
  const lowered = detail.toLowerCase();
  const isTDPlay = /\btouchdown\b|\bfor a td\b/i.test(lowered);
  if (!isTDPlay) return false;

  const events = play?.parsed?.events ?? [];

  // parser/TD events for this specific player
  if (
    events.some(
      (e) =>
        samePlayer(e.player, player) &&
        (/td/i.test(String(e.stat || "")) ||
          /touchdown/i.test(String(e.note || "")))
    )
  ) {
    return true;
  }

  // extracted scorer helper
  const who = extractTdScorerFromPlay(play);
  if (who && samePlayer(who, player)) return true;

  // text fallback if the TD call literally names them
  const last = canonName(player).split(" ").pop();
  if (last && lowered.includes(last) && /\btouchdown\b/i.test(lowered)) {
    return true;
  }
...
function getOUSideFromRaw(raw) {
  const m = String(raw?.market || "").toLowerCase();
  const d = String(
    raw?.details || raw?.side || raw?.ou || raw?.labelTop || ""
  ).toLowerCase();
...
function deltaForPlayerOnPlay(events, player, kind /* 'rush'|'rec'|'pass' */) {
  ...
  return (events || [])
    .filter(
      (e) =>
        samePlayer(e.player, player) &&
        set.has(String(e.stat || "").toLowerCase())
    )
...
function getScoreAt(play) {
  return {
    home: Number.isFinite(toNum(play?.pbp_score_hm))
      ? toNum(play.pbp_score_hm)
      : null,
    away: Number.isFinite(toNum(play?.pbp_score_aw))
      ? toNum(play.pbp_score_aw)
      : null,
  };
}
...
// helper so later bet types can't overwrite earlier pulse
function assignPulse(curPulse, newPulse) {
  return curPulse || newPulse || null;
}
...
export default function PlayByPlayVisualizer({
  ...
  const renderHomeLeft = useMemo(
    () => isHomeLeftForQuarter(current?.quarter),
    [current?.quarter]
  );
  const isHomeSide = useMemo(
    () => makeSideIsHomeFn(home, away),
    [home, away]
  );
  ...
  /** ---------------- Snap neighbors ---------------- */
  const prevSnapIdx = useMemo(
    () => findSnapIndex(enriched, i, -1),
    [enriched, i]
  );
  const nextSnapIdx = useMemo(
    () => findSnapIndex(enriched, i, +1),
    [enriched, i]
  );
  ...
  const evt = current
    ? classifyEvent(current.detail, current.down)
    : "OTHER";
  ...
  const nextChangesDrive =
    ...
    (nextSnap.startOfDrive ||
      (prevSnap?.posTeam &&
        nextSnap.posTeam &&
        nextSnap.posTeam !== prevSnap.posTeam));

  const activeSnap = current?.isSnap
    ? current
  ...
  const seriesHeadIdx = useMemo(
    () =>
      findSeriesHeadIndex(
        enriched,
        prevSnapIdx >= 0 ? prevSnapIdx : activeIdx,
        headIdx
      ),
    [enriched, prevSnapIdx, activeIdx, headIdx]
  );
  const seriesHeadSnap =
    seriesHeadIdx >= 0 ? enriched[seriesHeadIdx] : headSnap;

  const driveOffense =
    headSnap?.posTeam ||
    inferOffense(headSnap, isHomeSide, H, A);
  ...
  const xInFrame = (loc, homeLeftFrame) =>
    yardlineToPercentBySide(
      parseLocation(loc),
      isHomeSide,
      homeLeftFrame
    );
  const toRenderFrame = (pctFromFrame, homeLeftFrame) =>
    homeLeftFrame === renderHomeLeft
      ? pctFromFrame
      : 100 - pctFromFrame;
  ...
  const renderLosPrev = prevSnap
    ? toRenderFrame(
        xInFrame(
          prevSnap.location,
          isHomeLeftForQuarter(prevSnap.quarter)
        ),
        isHomeLeftForQuarter(prevSnap.quarter)
      )
    : 50;
  ...
  const renderLosNext = nextSnap
    ? toRenderFrame(
        xInFrame(
          nextSnap.location,
          isHomeLeftForQuarter(nextSnap.quarter)
        ),
        isHomeLeftForQuarter(nextSnap.quarter)
      )
    : renderLosPrev;
  ...
  const startX = (() => {
    if (evtClass === "KICKOFF") {
      ...
      const kPct = current?.location
        ? xInFrame(current.location, kFrame)
        : renderLosPrev;
  ...
  // Result marker and endzone pin logic
  const offenseOnPrevSnap =
    prevSnap?.posTeam ||
    inferOffense(prevSnap, isHomeSide, H, A);
  let resultEndzoneSide = null;
  if (scoreKind && offenseOnPrevSnap) {
    const prevFrameLeft = isHomeLeftForQuarter(prevSnap?.quarter);
    const dirPrev = dirForTeam(
      offenseOnPrevSnap,
      prevFrameLeft,
      H,
      A
    ...
    resultEndzoneSide =
      prevFrameLeft === renderHomeLeft
        ? endInPrevFrame
        : endInPrevFrame === "right"
        ? "left"
        : "right";
  }
  ...
  if (evtClass === "KICKOFF" || evtClass === "PUNT") {
    if (evtClass === "KICKOFF") {
      ...
      if (isTB) {
        const kickSide = parseLocation(current?.location)
          ?.side;
        const kickerIsHome = kickSide
          ? isHomeSide(kickSide)
          : null;
        const receivingIsHome =
          kickerIsHome === null ? null : !kickerIsHome;
        const receivingLeftInRender =
          receivingIsHome === null
            ? renderHomeLeft
            : receivingIsHome
            ? renderHomeLeft
            : !renderHomeLeft;
        resultEndzoneSide = receivingLeftInRender
          ? "left"
          : "right";
      } else {
  ...
  if (
    current?.fumble &&
    current.fumble.recoveredByTeam &&
    prevSnap?.posTeam
  ) {
    const offTeam = prevSnap.posTeam;
    if (
      current.fumble.recoveredByTeam === offTeam &&
      current.fumble.recoveryLoc
    ) {
      const recFrame = isHomeLeftForQuarter(current.quarter);
      const recPct = xInFrame(
        current.fumble.recoveryLoc,
        recFrame
      );
      resultEndzoneSide = null;
  ...
  if (scoreKind !== "XP" && evtClass !== "KICKOFF") {
    const baseSnap =
      evtClass === "PUNT"
        ? seriesHeadSnap || prevSnap || activeSnap
        : activeSnap || prevSnap || seriesHeadSnap;
    if (baseSnap?.location) {
      ...
      const off =
        baseSnap.posTeam ||
        inferOffense(baseSnap, isHomeSide, H, A);
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setI((prev) =>
        prev >= enriched.length - 1 ? prev : prev + 1
      );
    }, 250);
    return () => clearInterval(timerRef.current);
  }, [auto, enriched.length]);

  /** ---------------- Bet impact pulse / field flash ---------------- */
  const [impactFlash, setImpactFlash] = useState(null); // 'pos' | 'neg' | null
  ...
  useEffect(() => {
    ...
    const hDelta =
      nowScore.home != null && prevScore.home != null
        ? nowScore.home - prevScore.home
        : 0;
    const aDelta =
      nowScore.away != null && prevScore.away != null
        ? nowScore.away - prevScore.away
        : 0;
    const anyScoreThisPlay = hDelta > 0 || aDelta > 0;

    // we'll compute bet updates in one pass, but we WON'T mutate autoplay here
    // autoplay pause gets handled in a separate effect when pulses appear
    let localPosCount = 0;
    let localNegCount = 0;
    ...
    const updated = (prevState) =>
      prevState.map((c) => {
        ...
        let {
          ...
          hits = Number.isFinite(c.hits) ? c.hits : 0,
        } = c;

        let pulse = null;

        // 1. PLAYER YARD PROPS (rush/rec/pass OVER/UNDER)
        ...
        const yardDelta = deltaForPlayerOnPlay(
          events,
          player,
          kind
        );
        ...
        // 2. GAME TOTAL (OVER/UNDER total points)
        const isTotal =
          /total|over_under|game_total/.test(mkt) ||
          /over|under/.test(tstr);

        if (isTotal && anyScoreThisPlay) {
          const sideTotal =
            getOUSideFromRaw(raw) ||
            (tstr.includes("over")
              ? "OVER"
              : tstr.includes("under")
              ? "UNDER"
          ...
          if (sideTotal) {
            // any points help OVER, hurt UNDER
            const helpful = sideTotal === "OVER";
            pulse = assignPulse(
              pulse,
              helpful ? "pos" : "neg"
            );
        ...
        // 3. MONEYLINE / SPREAD
        const isMoneyline =
          /moneyline|ml\b/.test(mkt) ||
          tstr === "moneyline" ||
          tstr === "ml";
        const isSpread =
          /spread/.test(mkt) || tstr === "spread";

        if (
          (isMoneyline || isSpread) &&
          (hDelta !== 0 || aDelta !== 0)
        ) {
          const teamCode = getTeamCodeFromRaw(
            raw,
            home,
            away
          );
          if (teamCode) {
            ...
            const teamPoints = isHomeTeam
              ? hDelta
              : aDelta;
            const oppPoints = isHomeTeam
              ? aDelta
              : hDelta;

            if (teamPoints > 0) {
              pulse = assignPulse(pulse, "pos");
              localPosCount++;
            } else if (oppPoints > 0) {
              pulse = assignPulse(pulse, "neg");
              localNegCount++;
            }
          }
        }

        // 4. PLAYER TD MARKETS (first TD / anytime TD / N+ TDs)
        const isTdMarket =
          mkt.includes("td") ||
          tstr === "td" ||
          /(touchdown)/.test(tstr);
        if (isTdMarket) {
          ...
          const scoredNow = didThisPlayScoreFor(
            p,
            current
          );

          if (isFirst) {
            // logic: first TD market only resolves on first TD of game
            const isTDNow = /\btouchdown\b/i.test(
              String(current?.detail || "")
            );
            const tdOccurredEarlier = enriched
              .slice(0, i)
              .some((ply) =>
                /\btouchdown\b/i.test(
                  String(ply.detail || "")
            ...
            if (
              ...
              if (!tdOccurredEarlier && isTDNow) {
                ...
                const firstTDPlay = enriched.find(
                  (ply) =>
                    /\btouchdown\b/i.test(
                      String(ply.detail || "")
                    )
                );
                const our = didThisPlayScoreFor(
                  p,
                  firstTDPlay
                );
                status = our ? "won" : "lost";
    ...
    // net field flash (green if net positive bets > net negative bets)
    const net =
      localPosCount > localNegCount
        ? "pos"
        : localNegCount > localPosCount
        ? "neg"
        : null;
  ...
  }, [current?.idx, i, enriched, home, away, H, A, current]);

  // Pause autoplay WHENEVER any bet just pulsed,
  // and clear pulses from BetSlip after ~0.5s.
  useEffect(() => {
    const hasPulse = betUI.some(
      (b) => b.pulse === "pos" || b.pulse === "neg"
    );
    if (!hasPulse) return;

    // stop autoplay if we saw impact
    setAuto(false);

    // clear pulses from each bet card
    const t = setTimeout(() => {
      setBetUI((prev) =>
        prev.map((b) =>
          b.pulse ? { ...b, pulse: null } : b
        )
      );
    }, 550);
  ...
  if (!enriched.length)
    return <div style={wrap}>No play-by-play available.</div>;

  const downLabel =
    !current?.down || !current?.yds_to_go
      ? "—"
      : `${current.down}${ordinalSuffix(
          current.down
        )} & ${current.yds_to_go}`;
  const clockLabel = [
    current?.quarter ? `Q${current.quarter}` : "",
    current?.qtr_time_remain || "",
  ]
    .filter(Boolean)
    .join("  •  ");
  ...
  return (
    <div
      style={{
        ...bug,
        gridTemplateColumns:
          "max-content minmax(96px, 1fr) max-content",
      }}
    >
      <div style={{ ...bugTeam }}>
        <img
      ...
      <div style={bugCenter}>
        <div style={bugCenterRowTop}>
          <span style={bugClock}>
            {clock || "—"}
          </span>
        </div>
        <div style={bugCenterRowBot}>
          <span style={bugDn}>
            {downAndDist || "—"}
          </span>
        </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: active
          ? "radial-gradient(circle at 30% 30%, #fff, #ffe066)"
          : "transparent",
        boxShadow: active
...
const isProgressFlag = (v) =>
  String(v || "").toLowerCase() ===
  "progress";

function deriveProgress(b) {
  let current = Number.isFinite(
    Number(b.actual)
  )
    ? Number(b.actual)
    : toNum(
...
function createHeader(b) {
  ...
  const type = (
    ...
    .toLowerCase();
  const sel =
    b.selection ?? b.raw?.selection ?? "";
  const details = (
    b.details ?? b.raw?.details ?? ""
  )
    .toString()
    .toUpperCase();
  const threshold =
  ...
  if (
    ...
    return `${sel} ${
      OUMap[details] ?? details
    } ${threshold} ${displayText}`.trim();
  } else if (
    market === "player" &&
    type.includes("td")
  ) {
...
function Field({
  ...
  const ezWidth = totalInnerWidth * 0.1;
  ...
  const firstPx = Number.isFinite(
    firstDownX
  )
    ? yardToX(firstDownX)
    : null;

  const resultTranslateX =
    resultEndzoneSide === "left"
      ? leftEzCenter
  ...
  function trianglePoints(cx, cy, dir) {
    ...
    if (dir >= 0) {
      const apexX = cx + halfL,
        baseX = cx - halfL;
      return `${apexX},${cy} ${baseX},${
        cy - halfH
      } ${baseX},${cy + halfH}`;
    } else {
      const apexX = cx - halfL,
  ...
  return (
    ...
    >
      ...
      {/* Result dot */}
      <g
        transform={`translate(${resultTranslateX}, ${midY})`}
        style={{
          transition:
...
const bugCenterRowTop = {
  display: "flex",
  alignItems: "center",
  gap: 8,
};
const bugCenterRowBot = {
  display: "flex",
  alignItems: "center",
  gap: 8,
};
...
const circleBtnOn = {
  background:
    "linear-gradient(180deg, rgba(26,90,51,.9), rgba(26,90,51,.7))",
  boxShadow:
    "0 0 0 2px rgba(45,122,74,.35) inset, 0 4px 12px rgba(0,0,0,.35)",
};
const circleBtnOff = {
  background:
    "linear-gradient(180deg, rgba(10,22,16,.7), rgba(10,22,16,.55))",
  boxShadow:
    "inset 0 0 0 1px rgba(42,86,60,.6)",
};
...
const progressFillViz = {
  ...
  width: "0%",
  background:
    "linear-gradient(90deg, rgba(55,220,0,1), rgba(78,255,30,1))",
  boxShadow:
...
const pulseGreen = {
  ...
  borderColor: "#374151",
  background:
    "linear-gradient(180deg, rgba(22,60,38,.55), rgba(22,60,38,.35))",
  transition:
    "box-shadow 280ms ease, background 280ms ease, border-color 280ms ease",
};
...
const pulseRed = {
  ...
  borderColor: "#374151",
  background:
    "linear-gradient(180deg, rgba(60,22,22,.45), rgba(60,22,22,.25))",
  transition:
    "box-shadow 280ms ease, background 280ms ease, border-color 280ms ease",
};
